	...
	const icons = [
		{
			pathname: "/dashboard",
			icon: (
				<svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="25px" height="25px" viewBox="0 0 64 64">
					{/* ... Dashboard SVG path */}
				</svg>
			),
		},
		...
		<header className="bg-white min-w-[250px] h-screen relative">
			...
			<nav className="pr-7 py-7 ">
				<ul className="flex flex-col gap-5">
					{icons.map(({ pathname: iconPathname, icon }, index) => (
						<SidebarLink
							key={index}
							href={iconPathname}
							icon={icon}
							text={iconPathname.replace(/^\/|\/$/g, "")} // Extract meaningful text from pathname
