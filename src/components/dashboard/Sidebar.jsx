...
const Sidebar = () => {
	...
	const deleteDataPathname = "/settings/delete-data";
	...
	const icons = [
		...
		},
		{
			...
			icon: (
				<svg
					...
					height="24"
					viewBox="0 0 24 24"
					fill={pathname === settingsPathname || pathname === deleteDataPathname ? "#54696c" : "#4C4C4C"}
					xmlns="http://www.w3.org/2000/svg"
				>
					<path
		...
		{
			pathname: subscriptionsPathname,
			name: "Subscriptions",
			icon: (
		...
		<header className="bg-white min-w-[250px] h-screen relative">
			...
			<nav className="pr-7 py-7 ">
				<ul className="flex flex-col gap-5">
					{icons.map(({ pathname: iconPathname, icon, name }, index) => (
						<SidebarLink key={index} href={iconPathname} icon={icon} text={name} currentPath={pathname} />
					))}
				</ul>
